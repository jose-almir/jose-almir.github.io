...
function getCollection(dir, language = "pt") {
  ...
  const collectionData = files.map((filename) => {
  ...
  return collectionData
    ...
    .filter((p) => p.hidden === undefined && p.isPrivate !== true);
}
...
languages.forEach((lang) => {
  const posts = getCollection(postsDir, lang);
  const projects = getCollection(projectsDir, lang);
  ...
  projects.forEach((project) => {
    feed.item({
      ...
      date: project.date || new Date().toISOString(), // Use current date if missing from project
      categories: ["project"],
    });
  });
